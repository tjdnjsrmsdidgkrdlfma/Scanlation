...
- **`image_min_pixels`** — 이제 recognize에 남은 유일한 속도 레버다. 동시성·`-c`가 둘 다 음성으로 닫혔고, 크롭당 vision 프롬프트 213~230 토큰에 prefill이 per-crop의 2/3라 시간이 실제로 여기 있다. mmproj의 바닥값(147,384)이 작은 크롭까지 188 토큰으로 끌어올린다 — 낮추면 빨라지지만 작은 SFX 크롭의 오독 위험이 있어 정확도와 같이 재야 한다.
