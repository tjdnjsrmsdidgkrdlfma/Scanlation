...
- **런타임 교체(ONNX Runtime)는 2.33x인데 ❌ 미채택이다** — 같은 21페이지에서 torch 237.4 → **ORT 102.0ms**([bench_detect_runtime.py](packages/scanlation-server/tools/bench_detect_runtime.py), 전처리 텐서 공유·스레드 8 동일)이고 **INT8은 오히려 느리다**(154.9ms). 막은 건 속도가 아니라 **출력**이다: `samples/` 17장으로 표본을 넓히자 3장이 불일치했고 그중 한 장에서 **하단 컷의 대사를 통째로 놓쳤다**(torch가 0.68~0.80으로 잡는 박스 3개). export된 ONNX가 safetensors와 다른 모델이라 생기는 차이고, **검출 누락은 곧 번역 누락**이다. 이득 쪽도 크지 않다 — serial 페이지의 8.2%, 동시성 4에서 5~6%이며 그 위 translate 천장(1.64 req/s)은 못 넘는다. **남은 채택 사유는 속도가 아니라 torch·torchvision 제거**이고, 이미지 크기가 문제가 될 때 다시 본다([detect-runtime.md §3](packages/scanlation-server/tools/detect-runtime.md)).
...
**그런데 그 격차는 대역폭 효율이 아니다.** KV(할당 4096 전폭 0.88GB)를 전부 낭비로 치고 대역폭을 이론 peak로 쳐도 메모리는 토큰당 11.2ms 중 **3.0ms(27%)**뿐이다. 소거하면 **연산 바운드 아님**(150W 캡 비용 0)이고 **백엔드 문제도 아니다**(HIP 88.1 ≈ Vulkan 89.3).

**커널 프로파일이 범인을 확정했다 (2026-08-06).** `GGML_VK_PERF_LOGGER=1` 계측에서 토큰당 커널은 **1,172개**, 커널 GPU 시간 합은 **12.378ms**로 스텝의 **94.5%**다 — **커널 밖 디스패치 갭은 5.5%**뿐이라 "디스패치 지연이 범인" 가설은 기각된다. 시간은 커널 안에서, 그중 **잔 커널**에서 나간다: `RMS_NORM_MUL`이 211회에 **2.02ms(16.4%)**로 최대 단일 항목이고 실효 대역폭이 1.1GB/s다. 반대로 굵은 커널은 멀쩡하다 — 출력 로짓(415MB)을 688µs에 = **603GB/s(peak의 59%)**로, 9060 XT의 65%와 같은 영역이다. 상세·재현은 [translate-gpu-mi50.md](packages/scanlation-server/tools/translate-gpu-mi50.md).

| 레버 | 회수 가능분 |
|---|---|
| `-c` 하향 — ❌ 미채택(요청 크기에 상한이 없다) | 스텝의 **3.6%** (`FLASH_ATTN_EXT` 실측) |
| 서브밋 배칭·동기화 축소 | 스텝의 **5.5%** (디스패치 갭 전부) |
| 잔 커널 융합 — 상류 llama.cpp 몫 | 잔 커널이 스텝의 **~1/3** |
...
- **수 분 지속 부하는 맨팬으로 못 잡는다.** 3분 풀로드에서 junction이 50초 만에 96°C에 닿고, 팬 상한을 5,208 → 7,075rpm으로 올려도 1°C밖에 안 내려간다. **전력 캡도 레버가 아니다** — 120W는 10초를 벌 뿐이고 100W라야 135초(토큰 90.7 → 80.5 t/s)지만 그래도 3분을 못 버틴다. 열이 나는 양이 아니라 **빼내는 양이 병목**이라 쉬라우드(Task 3)가 답이고, 그때까지 운영은 150W를 유지한다. MI50은 **온도 스로틀이 없어** 완충도 없다 — 지속 부하를 돌리게 되면 온도 가드(Task 5)가 먼저다([cooling-mi50-fans.md](packages/scanlation-server/tools/cooling-mi50-fans.md)).
