...
> - **다중 패키지 분리** — `server/` 단일 패키지를 `packages/`의 여러 pip 패키지로 분리: 공유 계약 `scanlation-sdk`(contracts·context·prompt·testing) + 코어 `scanlation-server`(dummy 엔진만 번들) + 엔진별 패키지(`scanlation-{comic-text-and-bubble-detector,manga-ocr,ollama,llama-cpp}`). 코어는 엔진을 전혀 모르고 **`entry_points`로만 발견**(`registry._BUILTIN` 하드코딩 맵 제거) — "설치한 패키지 = 탑재 엔진". 아래 §의 `server/plugins/*`·`app.contracts`·`plugins.llm_prompt` 경로 참조는 이 구조로 이동됨(계약 = `scanlation_sdk`).
...
> ⚠ **하드웨어·토폴로지 divergence.** recognize GPU는 **9060 XT 16GB**(gfx1200/RDNA4), **MI50 32GB(HBM2) 도입 예정**. 방향은 **GPU 역할 분리** — recognize(PaddleOCR-VL)=9060 XT, translate(Gemma)=MI50 — 라 "한 카드에서 Ollama와 OCR이 VRAM 경합"하던 원 전제가 풀린다(MI50가 HBM2 대역폭 + 넉넉한 VRAM으로 KV캐시 희생하며 Gemma 다중 인스턴스). 정확도 때문에 recognizer를 manga-ocr→**PaddleOCR-VL**로 전환하면 파이프라인이 **recognize-bound**가 된다(recognize-only 68.6s = translate 31.7s의 2.2배, run_report 실측). 그래서 recognizer 처리량을 **멀티워커**(W개 프로세스 각 B=1로 한 페이지의 crop을 fan-out, 측정 W=4 1.38x)로 끌어올린다 — 린 코어 구현(`app/recognize_pool.py`), env `SCANLATION_RECOGNIZE_CONCURRENCY`·`/admin` 플러그인별, 기본 W=1 = off(기존 per-crop). 배치는 폐기(straggler + vision O(n²)). `gpu_lock`→`InferenceGate`(reader/writer 게이트, per-recognizer K)·크로스이미지 오버랩·**3-스테이지 분리**(detect=게이트 밖 CPU / recognize=게이트 안 GPU / translate=게이트 밖)는 **구현됨**. **남은 MI50 단계는 배포** — ollama(gemma)를 MI50에 핀하면 recognize(9060)∥translate(MI50) 병렬이 배포만으로 켜진다(translate가 이미 게이트 밖이라 코드 변경 불필요). 상세: [recognize-gpu-speed.md](packages/scanlation-server/tools/recognize-gpu-speed.md). *TODO — 진행 표시 순서: 20장 동시 처리 시 앞에서부터 완성돼 한 장씩 보이게 하려면 요청에 페이지 순번 필드(확장 변경) + detect·recognize 순번 우선 스케줄이 필요(현행은 detect 직렬+recognize FIFO라 대략 도착순).*
...
rtdetr = "scanlation_rtdetr.plugin:RTDetrDetector"
