...
**남은 리팩토링:** R7~R9(대형 파일 분할). (R5는 핵심 완료 — 확장 defaults 통합은 R9로 이월.)
...
1. **R7 → R9** — 대형 파일 분할 (R7은 R2가 남긴 `compare_models.py`의 어휘 잔여를 함께 처리). R9는 R5가 이월한
   확장 defaults 통합(엔드포인트 3중복·`minImageDim` DEFAULTS gap)·`"번역 실패"` i18n을 함께 처리
