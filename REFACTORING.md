...
**남은 리팩토링:** R8~R9(대형 파일 분할). (R7 완료 — R5가 이월한 확장 defaults 통합은 여전히 R9.)
...
1. **R8 → R9** — 대형 파일 분할 (R7 완료). R8은 `app/web/app.js`(994줄)의 i18n 테이블(:14-218) 분리. R9는 R5가
   이월한 확장 defaults 통합(엔드포인트 3중복·`minImageDim` DEFAULTS gap)·`"번역 실패"` i18n을 함께 처리
