...
1. ~~**R2** — 어휘 정리~~ ✅
2. **R1** — 벤치 공통 모듈 추출(순수 중복 제거). `load_crops(deskew=...)`가 B5를 계약으로 승격시킨다
3. **B5+B6** — threads를 deskew 크롭으로 뒤집고 리포트의 하드코딩된 결론 산문을 제거. **측정값이 바뀌므로 벤치를 돌릴 수 있는 GPU 호스트에서**, `tools/*.md`의 1.27x vs 1.8x 결론 재검토와 함께
4. **R3** — SDK 계약 표면
5. **B1~B4** — 버그 4건 (각각 개별 판단; B1은 세로 읽기 순서 구현 여부 결정 필요)
6. **R4 → R6** — 설치기 통합, 플러그인 보일러플레이트
