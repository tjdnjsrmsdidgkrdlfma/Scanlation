...
1. **R6** — 플러그인 보일러플레이트를 SDK로. 5개 패키지에 걸치고, **가중치 없는 머신에선 엔진 스모크가 skip되어 검증이 얕아진다**
2. **R5** — 하드코딩 → `/admin`. 새 설정 필드가 늘어나므로 handshake·확장·i18n 동시 수정
3. **R7 → R9** — 대형 파일 분할 (R7은 R2가 남긴 `compare_models.py`의 어휘 잔여를 함께 처리)
