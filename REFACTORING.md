...
코어 테스트 58개 → **77개**. SDK가 자체 스위트를 얻었다(순수 헬퍼 5개, 가중치 불필요).
...
**남은 리팩토링:** R5(하드코딩→`/admin`), R7~R9(대형 파일 분할).
...
1. **R5** — 하드코딩 → `/admin`. 새 설정 필드가 늘어나므로 handshake·확장·i18n 동시 수정
2. **R7 → R9** — 대형 파일 분할 (R7은 R2가 남긴 `compare_models.py`의 어휘 잔여를 함께 처리)
