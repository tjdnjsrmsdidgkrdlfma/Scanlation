...
**09-24가 보탠 것 — 소진은 결정론적이고 저절로 끝난다.** 09-14은 강제 전원 차단으로 끝나 끝을 못 봤는데,
09-24 두 버스트는 손대지 않았는데도 같은 수치로 끝났다 — 타임아웃 81회, 10초 간격, fence 158개, 13분 40초:

| | 링 | 첫 타임아웃 | 마지막 | 횟수 | fence |
|---|---|---|---|---|---|
| 09-24a | `comp_1.0.1` | 14:47:03 | 15:00:43 | 81 | 76969 → 77127 |
| 09-24b | `comp_1.1.0` | 15:13:37 | 15:27:16 | 81 | 103092 → 103250 |

걸린 순간 큐에 남은 fence를 리셋이 2개씩 밀어내 81회에 비운다는 뜻이고, 마지막 fence가 빠져야 llama-server가
`failed to encode mtmd batch for chunk idx = 5`를 내고 슬롯을 놓는다. **전원을 내릴 필요는 없다** — 요청만
끊기면 카드는 13분 반 뒤 0% idle로 돌아온다. 정지 시간은 클라이언트가 얼마나 오래 재시도하느냐로 정해진다.
링도 둘이 갈려서(`comp_1.0.1`/`comp_1.1.0`) 특정 큐의 고장은 아니다.

**그 13분 반은 `MAX_NODES_PER_SUBMIT=8`이 만든 것일 수 있다.** 08월 사고의 타임아웃 횟수는 8·32·7회인데
09-24는 81회다. 노드를 8개씩 쪼개면 같은 그래프가 더 많은 제출로 나뉘고 소진은 제출 수에 비례한다. 08월 건은
