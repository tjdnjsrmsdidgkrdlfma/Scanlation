...
so the developer's real data/state.json is never touched. AppState.__init__
also writes context.device, so that is restored too.
"""
...
def test_state_json_roundtrip():
    saved_base, saved_dev = context.base_dir, context.device
    try:
        ...
        context.base_dir, context.device = saved_base, saved_dev
