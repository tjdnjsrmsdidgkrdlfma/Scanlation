# detect 런타임 — CPU 레버는 소진, ONNX는 2.33x인데 대사를 놓치고, GPU는 카드가 못 버틴다
...
| ONNX Runtime CPU | **102.0ms** | 미측정 | 대사를 놓치는 페이지가 있다(§3) | ❌ **속도로는 미채택** |
| ONNX Runtime CPU (int8) | 154.9ms | — | 동일 | ❌ fp32보다 느리다 |
| torch GPU (`cuda:1`) | 98.8ms | **151.5ms** | torch와 동일 | ❌ **GPU hang — 못 쓴다** |

**속도만 보면 GPU가 최고이고 실제로 파이프라인 detect를 302 → 151ms로 정확히 반토막 냈다. 그런데 같은 런에서 GPU가 두 번 hang했다.** recognize와 카드를 공유하는 한 쓸 수 없고, 옮길 카드도 없다(MI50은 torch가 안 돈다).

**ONNX는 카드를 전혀 안 건드리면서 GPU와 같은 속도를 내지만, 대사를 놓치는 페이지가 있어 미채택이다.** 이득의 크기가 그 위험을 못 산다 — detect는 serial 페이지의 14.4%라 2.33x가 페이지 total을 8.2% 줄이고, 운영값인 동시성 4에서는 페이지 지연 3.06 → 2.89초로 **5~6%**다. 그 위에 translate 포화 천장 **1.64 req/s**가 있어 detect를 0으로 만들어도 넘지 못한다. **detect는 병목이 아니라 천장까지 남은 20% 격차의 일부**고, 검출 누락은 곧 번역 누락이다.

**남은 채택 사유는 속도가 아니라 의존성이다** — ONNX 경로는 플러그인에서 `torch`+`torchvision`을 걷어낸다([pyproject.toml](../../scanlation-comic-text-and-bubble-detector/pyproject.toml)). 활성 recognizer가 llama.cpp인 배포에서는 plugins 볼륨에서 torch가 통째로 빠진다. 이미지 크기가 문제가 될 때 다시 볼 항목이다.
...
**표본을 넓히면 차이가 커진다.** 위 21페이지는 한 작품이라 화풍이 하나다. `samples/`의 17장(노이즈·많은 글씨·말풍선X·손글씨·평범)으로 같은 A/B를 돌리면 106 vs 105박스에 **3장이 불일치**하고, 좌표는 오히려 더 붙는다(최악 6.5px).

| 페이지 | 차이 | 무엇 |
|---|---|---|
| `145641047_p0` | torch 6 → **ORT 3** | 하단 오마케 컷의 대사 2개와 효과선. torch가 0.68~0.80으로 잡는데 ORT는 통째로 못 본다 |
| `145486034_p9` | torch 2 → ORT 3 | 효과음 하나를 ORT만 잡는다(ORT 쪽 이득) |
| `146404075_p0` | torch 3 → ORT 4 | 타이틀 박스 안에 중첩된 여분 |

**누락이 한 페이지에 3개 몰린다는 게 판정을 갈랐다.** 순증감은 -1이라 총계로는 21페이지와 같아 보이지만, 실제로 사라지는 것은 컷 하나 분량의 대사다.
...
- **ONNX는 이미지 크기가 문제가 될 때 다시 본다** — 속도로는 닫혔고(§결론), 남은 사유는 torch·torchvision 제거뿐이다. 그때 설계 판단이 하나 남는다: 엔진을 나눌지 같은 엔진의 옵션으로 둘지, 그리고 onnxruntime 의존성을 받을지.
