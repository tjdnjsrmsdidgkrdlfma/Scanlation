...
**보충 — translate 단독 스케일링 (llama-server 직타, 2026-07-17).** 위 스윕은 파이프라인 처리량이라 translate 슬롯이 공급(recognize 직렬) 탓에 덜 채워진다 — MI50 자체의 동시성 이득을 격리하기 위해 같은 배치 body 12요청을 `xargs -P`로 직타(동일 프롬프트라 prefill은 슬롯 캐시로 무시 수준 = 사실상 decode 스케일링):

| P | 12요청 벽시계 | 처리량 | 요청당 실효 | 스케일 |
|---|---|---|---|---|
| 1 | 8.02s | 1.50 req/s | 667ms | 1.00x |
| 2 | 5.66s | 2.12 req/s | 471ms | ×1.42 |
| 4 | 4.93s | 2.43 req/s | 411ms | ×1.63 |

- **수확체감이 가파르다(4슬롯에 ×1.63)** — decode는 대역폭 바운드라 슬롯들이 같은 HBM을 나눠 쓰고, 배칭 이득은 가중치 읽기 공유분에 그친다. 요청당 지연은 그만큼 늘어난다(667→~1640ms).
- 파이프라인 스윕의 ×2.13 > 단독 ×1.63 — 초과분의 원천은 슬롯 스케일링이 아니라 **스테이지 간 겹침**(translate가 detect+recognize와 겹침).
- **동시성 4의 파이프라인 translate 실효 424ms/페이지 ≈ 단독 천장 411ms** — 공급이 끊겨 슬롯이 2~3개만 차 있어도 수확체감 덕에 처리량은 이미 천장이었다. 스윕은 translate 용량을 저평가하지 않았고, **9060 XT 전환 후 공급이 포화돼도 MI50 천장은 ~2.4 배치/s(요청당 ~411ms)가 기준**이다(그때 병목은 다시 translate로 복귀 예상, 열도 지금보다 심해짐 — 냉각 보강 전 동시성 4 금지 결론 유지).
