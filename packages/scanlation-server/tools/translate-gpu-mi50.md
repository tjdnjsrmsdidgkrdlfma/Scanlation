...
**KV를 넣으면 격차가 조금 줄지만 사라지진 않는다.** 위 계산은 가중치만 센 값이다. 이 모델의 KV는 컨텍스트 토큰당 ~210KB이고 `/props` 기준 슬롯당 `n_ctx`는 **4096**(`-c 16384 --parallel 4`)이라, 스텝마다 할당 폭 전체를 읽는다면 **+0.25~0.88 GB/token**이 붙는다(SWA 레이어를 좁은 링버퍼로 잡는지에 따라 갈린다). 그러면 천장이 ~330~420 t/s, 실측 비율이 21~24%가 된다.

**`-c` 16k→32k = -42%가 이 KV 항의 서명이다.** 시퀀스 길이가 같은데 할당 폭만 두 배가 됐을 때 그만큼 느려진다는 건, 스텝 시간의 상당 부분이 **비어 있는 KV를 읽는 데** 간다는 뜻이다. 그래서 다음 레버는 `-c` 하향이고, 선행 조건은 페이지당 실제 프롬프트 토큰 분포를 확인하는 것이다(지금 백스톱이 `--n-predict 1024`라 슬롯당 2048은 빠듯할 수 있다).
