...
**남는 건 지연 — 커널 디스패치·동기화다. (⚠️ 여기서부터 가설, 미측정)** 이 모델은 디스패치 밀도가 높다: 레이어당 norm/scale 텐서만 10개(각각 2816 float만 만지는, 유효 대역폭 0인 커널)이고, dense FFN(`ffn_gate/up/down`)과 routed expert(`ffn_*_exps`)를 **둘 다** 가지며, hidden이 2816으로 작아 커널 하나가 하는 일이 적다. 레이어당 30~40 연산 × 30 레이어 ≈ 토큰당 1,000회 남짓이면 8.2ms ÷ 1,000 = **디스패치당 ~8µs**다.

방증: 같은 카드에서 **dense gemma-2-9b Q4_K_M은 peak의 28%**(53.6 t/s)로, MoE인 이 모델의 19%보다 효율이 높다. dense가 더 굵고 적은 커널로 같은 바이트를 옮긴다는 것과 방향이 맞는다. **확증은 디스패치 단위 프로파일**(`rocprof` / `GGML_VULKAN_PERF_LOGGER`)로 커널 수와 커널당 시간을 직접 세는 것이고, 아직 안 했다.

### 회수 가능한 몫은 작다

`-c` 하향으로 되찾을 수 있는 건 **KV 항 0.86ms = 스텝의 8%**뿐이다. 73%는 설정으로 못 건드린다.
