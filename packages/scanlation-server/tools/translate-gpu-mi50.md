...
1. ~~**스캔레이션 서버 연결.**~~ **완료 (2026-07-16).** 플러그인 설치·엔드포인트 배선(§통합 시도 1·2), 폭주 확정·수정(§폭주 원인 확정 — `dry_multiplier` 0.8), 챕터 재검증(21/21·terse)까지. 남은 건 **동시성 2/4 스윕**뿐. (plugin: [scanlation-llama-cpp](../../scanlation-llama-cpp/scanlation_llama_cpp/plugin.py), OpenAI 호환 `/v1/chat/completions`, 모델 `unsloth/gemma-4-26B-A4B-it-qat-GGUF`.)
