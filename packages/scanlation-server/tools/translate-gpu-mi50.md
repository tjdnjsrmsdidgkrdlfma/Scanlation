...
> **상태 (2026-07-16):** **폭주 원인 확정·수정 — greedy 반복 루프.** 교성 텍스트(07.jpg `イ．．．くぅぅぅ〜〜〜んっっ`)가 "이... 으으으…"로 슬롯 컨텍스트가 찰 때까지 반복 — EOS도 grammar도 못 막는 유형(경위·로그·처방은 §폭주 원인 확정). 방어 3겹 배치: 플러그인 **`dry_multiplier` 0.8**(근본) + 유닛 **`--n-predict 1024`**(백스톱) + **전력 캡 150W**(decode 89~95 t/s 실측, 비용 ≈ 0). 2차 hang 자체는 콜드 부팅으로 복구(카드 생존) — 인과는 §4(무냉각 지속 부하 → 과열 → PCIe 버스 이탈, SIGKILL은 증상). **남은 것: 플러그인 재설치 → 챕터 재검증(21/21·terse·mem<85°C) → 동시성 2/4 스윕.**
...
→ [플러그인](../../scanlation-llama-cpp/scanlation_llama_cpp/plugin.py)에 **`dry_multiplier` 옵션(기본 0.8, OPTION_SCHEMA→/admin)** 추가 — 배치·단일 공통 body로 전송. DRY는 "재확장되는 반복 시퀀스"에만 벌점이라 JSON 구조 토큰·일반 문장 무손상. 단위테스트 green. **플러그인 재설치 필요**(§배포 검증 절차) → 챕터 재주행이 종료 조건(21/21·전 요청 수십 토큰·mem<85°C).
...
1. **스캔레이션 서버 연결.** 플러그인 설치·엔드포인트 배선은 됨(§통합 시도 1·2), 폭주 원인도 확정·수정됨(§폭주 원인 확정 — `dry_multiplier` 0.8). 남은 건 **플러그인 재설치 후 챕터 재검증**(21/21·전 요청 terse·mem<85°C)과 동시성 2/4 스윕. (plugin: [scanlation-llama-cpp](../../scanlation-llama-cpp/scanlation_llama_cpp/plugin.py), OpenAI 호환 `/v1/chat/completions`, 모델 `unsloth/gemma-4-26B-A4B-it-qat-GGUF`.)
...
7. **폭주 방어선.** 근본 수정은 플러그인 `dry_multiplier` 기본 0.8(§폭주 원인 확정). 전역 백스톱은 유닛 `--n-predict 1024`(plugin 상태와 무관 — 클램프 실동작 확인은 챕터 재검증에서). 요청 단위 `max_tokens`(plugin 옵션 + `/admin`)는 필요해지면 추가(선택).
