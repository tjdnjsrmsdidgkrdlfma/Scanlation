...
- 실측 프롬프트는 **88~172 토큰**, 요청당 총 컨텍스트 **122~347 토큰**인데(서버 슬롯 로그) 슬롯당 `n_ctx`는 **4096**이다(`/props`). **90% 이상이 빈 폭이다.**
- `-c 16384` → **`-c 8192`**(슬롯당 2048)는 무조건 안전한 드롭인이다. 관측 최대 347에 `--n-predict 1024` 백스톱을 얹어도 들어간다. 더 내리려면 `--n-predict`를 먼저 낮춰야 한다(실측 생성은 34~116 토큰).
