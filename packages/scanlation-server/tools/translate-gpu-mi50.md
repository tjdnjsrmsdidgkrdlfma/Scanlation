...
- **확인**: 부하 중 `rocm-smi --showtemp --showpower --showclocks`. idle 온도는 무의미 — 추론을 돌리며 junction 온도·클럭을 본다. 클럭이 떨어지면 스로틀링.
- **열린 관측**: 벤치 중 decode가 §실측의 89 → 33 t/s 부근으로 낮게 관측된 적이 있는데, 배칭 per-slot rate인지 **열 스로틀링**인지 미확정 — 쿨링 확보 후 온도 보며 재확인 대상.
...
- **이건 하드웨어 고장이 아니라 알려진 소프트 한계다** (한참 헤맨 지점이라 못박음). GPU는 자기 명령을 비동기로 실행하는 별개 프로세서라, 작업 중 프로세스가 죽으면 커널은 "GPU가 그 VRAM을 다 썼다"는 fence를 못 받아 안전하게 반납을 못 한다(그래서 프로세스가 없어도 VRAM이 물림). `--gpureset`도 hung 컨텍스트를 **항상 못 푼다** — AMD 자체 트래커가 명시: `rocm_smi_lib` #85 *"reset will not always work, depending on the manner in which the GPU is hung"* (gpureset이 success를 찍고도 리부트 필요). **콜드 부팅만이 GPU 펌웨어를 재-POST해 확실히 되돌린다.** 판단 근거: 직전까지 정상 작동 + used 값이 죽은 프로세스 할당량과 byte 단위로 일치 + 카드가 여전히 정상 열거. (링크는 §관련.)
...
7. **폭주 방어선: `max_tokens` 옵션 (선택).** 서버측 하드 캡으로 폭주 GPU 소모·백로그를 격리(§폭주 격리). plugin이 배치/단일 요청에 `max_tokens`를 싣도록 + env 기본 + `/admin` 노출(하드코딩 금지). 값은 `n_ctx_slot`보다 낮게(1024급). **폭주 근본 원인 확정·수정이 우선**이고 이건 보조 방어선.
