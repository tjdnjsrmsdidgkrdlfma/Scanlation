...
## 10. `image_min_pixels` — 측정 완료: 속도는 벌지만 출력의 절반이 바뀐다

§6이 "잘못 핀된 서버에서만 재봤다"고 남겨둔 마지막 전처리 노브를 쟀다(2026-08-05). **속도 이득은 실재하는데 정확도 대가가 커서 채택하지 않는다.**
...
- ~~**남은 전처리 노브**~~ **전부 소진** — 캡(§7)·동시성/컨텍스트 폭(§9)·`image_min_pixels`(§10)가 다 닫혔다. §6의 미측정 목록에서 남은 건 **mmproj 양자화**뿐이다(지금 BF16 882MB). prefill이 per-crop의 2/3인데 그게 연산 바운드인지 weight read 바운드인지 아직 안 갈랐다 — 후자라야 이득이 난다.
