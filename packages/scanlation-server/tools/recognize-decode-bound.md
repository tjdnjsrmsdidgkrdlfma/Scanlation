...
두 문서에 흩어진 단계들을 한 줄에 세운 것. 같은 42 crop 기준이고, **누적은 실측된 첫 점(flash-OFF GPU)에 건다.**

| # | 단계 | crops/sec | 직전 | 누적 | 출처 |
|---|---|---|---|---|---|
| 1 | GPU, flash OFF | **0.094** | — | **1x** | [recognize-gpu-speed.md](recognize-gpu-speed.md) |
| 2 | **+ AOTriton flash attention** | 0.345 | **3.7x** | 3.7x | 〃 — env 한 줄. sdpa가 math 폴백 대신 flash 커널을 탄다 |
| 3 | + 해상도 캡 150k / `pow2` | 0.58 | 1.7x | 6.2x | 〃 — vision 토큰 감소 |
| 4 | + 멀티워커 W4·K2 | ~0.77 | 1.3x | 8.2x | 〃 — 호스트 오버헤드가 만든 GPU 유휴를 다른 워커가 채운다 |
| 5 | **llama.cpp로 런타임 교체** | 1.274 | 1.7x | 13.6x | §5 — decode의 ~90%인 호스트 오버헤드를 계층째 제거 |
| 6 | **+ GPU 핀을 env로** | **5.585** | **4.4x** | **59x** | §6 — vision 인코더가 옆 GPU로 새던 것 |
| 7 | 현재 — `box` / 300k | 4.277 | 0.77x | **46x** | §7 — **일부러 되돌린 단계.** 절단되던 크롭을 산다 |

> CPU 실행은 `~60s/crop`(≈0.02 crops/sec)이나 이건 플러그인 docstring의 어림값이라 배수 산정에서 뺐다. 크기만 말하면 **크롭 하나에 1분 → 1초에 4개**다.
