...
`--cool-cmd`가 이걸 푼다. 매 요청 직전에 [wait-cool.sh](../../../deploy/README.md) 류의 프로브를 호출해 카드가 식을 때까지 막으면 컷이 아예 발동하지 않고 한 패스로 완주한다(dense 17페이지에 25~29분, 대부분이 대기). 두 가지가 load-bearing이다:

- **프로브는 GPU 호스트에서 돌려야 한다.** 워크스테이션에서 ssh로 부르면 요청마다 왕복이 붙어 페이싱이 늦고, 그 사이 카드가 컷에 닿는다.
- **판단은 표본 하나로 하지 않는다.** 이 junction 센서는 연속 읽기 사이에 10~20°C씩 튄다. 90°C로 도는 카드가 71°C를 한 번 보고하므로, 단일 표본 게이트는 뜨거운 카드에 작업을 통과시키고 단일 표본 컷은 스파이크 하나에 백엔드를 죽인다. 게이트는 여러 표본의 최댓값으로, 컷은 연속 N회로 판단한다.
