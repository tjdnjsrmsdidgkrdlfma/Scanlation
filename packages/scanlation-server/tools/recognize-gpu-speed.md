...
| 해상도 캡 (150k px) | 1.66x | 24개 중 10개 출력 변경(일부는 실제 오독), 더 낮은 캡에선 줄 소실 | 보류 — 다운스케일 *방식*(정수배·28그리드) 개선 후 재측정 |
...
### 판정 보류 — 다음은 "얼마나 줄이냐"가 아니라 "어떻게 줄이냐"

- **1.66x는 작지 않다.** recognize가 이제 translate와 대등한 비중이라 end-to-end에 그대로 실린다.
- **150k에선 줄 소실이 관측되지 않았다**(잘림은 200k·50k). 다만 손상의 **비단조성** 때문에 "150k는 안전"을 보장하지는 못한다.

**다음 실험 — 다운스케일 *방식*.** 지금 `_cap_pixels`는 **임의 배율 + LANCZOS**다. 두 가지가 의심된다:

1. **임의 배율 리샘플링**은 얇은 획·`・・・` 점·작은 가나에 링잉/에일리어싱을 남긴다. **정수배(1/2, 1/3) box-average**는 출력 픽셀이 k×k 블록의 정확한 평균이라 훨씬 깨끗하다.
2. dynamic-res 프로세서는 **patch14·merge2 = 28px 그리드**에 맞춰 내부에서 다시 리사이즈한다. 우리 결과가 28의 배수가 아니면 **리샘플이 두 번** 걸려 아티팩트가 겹친다. 목표 크기를 **28의 배수로 스냅**하면 그 두 번째가 사라진다.

같은 캡에서 방식만 바꿔 비교하면(`area`(현행) vs 정수배 box vs 28-그리드 스냅), 손상이 **해상도 탓인지 리샘플링 탓인지** 갈린다. 리샘플링 탓이면 1.66x를 손상 없이 가져갈 수 있다.
