...
const I18N = {
  ko: {
    ...
    "optconc.label": "인식 동시 프로세스",
    "optconc.desc": "이 인식 플러그인을 여러 워커 프로세스로 병렬화(각 B=1, GPU 유휴 회수). VRAM은 이 값에 비례합니다. 비우면 서버 기본값, 1 = 풀 미사용.",
    "optgpuconc.label": "동시 처리 이미지 수",
    "optgpuconc.desc": "이 인식 플러그인으로 detect+recognize를 동시에 돌릴 이미지 수. 여러 이미지의 crop이 공유 워커풀을 함께 채워, 한 페이지 crop 수가 워커 수보다 적어도 풀을 채웁니다. 비우면 서버 기본값, 1 = 직렬(기본). 위 '인식 동시 프로세스'가 1보다 클 때(풀 사용)만 의미가 있습니다.",
    "btn.save": "저장",
    "prompt.h2": "번역 프롬프트",
    ...
    "tab.stats": "통계",
    "stats.h2": "통계",
    "stats.hint": "페이지·crop 처리 통계 (벤치 제외)",
    "stats.pages": "페이지 지표",
    ...
    "behavior.idleUnload.desc": "로컬 torch 엔진(탐지·인식)을 이 시간(분) 동안 안 쓰면 VRAM에서 내려, 작업 사이에 GPU를 붙잡고 있지 않게 합니다(ollama 번역기는 별도 프로세스라 무관 — 그쪽은 OLLAMA_KEEP_ALIVE가 관리). 0 = 안 내림(상주 유지).",
