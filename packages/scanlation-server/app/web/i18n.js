...
const I18N = {
  ko: {
    ...
    "optconc.desc": "이 인식 플러그인을 여러 워커 프로세스로 병렬화(각 B=1, GPU 유휴 회수). VRAM은 이 값에 비례합니다. 같은 수의 이미지가 detect+recognize를 동시에 지나며 서로의 crop으로 풀을 채웁니다. 비우면 서버 기본값, 1 = 풀 미사용(직렬).",
    "btn.save": "저장",
    "prompt.h2": "번역 프롬프트",
    ...
    "plugins.restartNeeded": "새 코드는 서버를 재시작해야 적용됩니다 (실행 중인 프로세스가 옛 모듈을 들고 있음).",
    ...
    "stats.filter": "엔진 조합",
    "stats.filter.all": "전체 (조합 섞임)",
    "stats.filter.desc": "조합마다 recognizer 속도가 크게 달라, 전체 합산 평균은 어느 조합도 대표하지 않습니다. 하나씩 골라 보세요.",
    "stats.pages": "페이지 지표",
    ...
    "behavior.minFont.label": "최소 글자 크기 (px)",
    "behavior.minFont.desc": "번역문은 박스 안에 들어가는 가장 큰 크기로 자동 조절되며, 이 값보다 작아지지는 않습니다. 이 크기로도 넘치는 글은 잘리고, 마우스를 올리면 전체 문장이 보입니다. 확장이 접속 시 이 값을 받아 적용합니다.",
    ...
    "behavior.idleUnload.label": "인식·탐지 모델 유휴 언로드 (분)",
    ...
    "toast.reinstalled": "{name} 재설치됨 — 서버 재시작 필요",
  ...
  en: {
    ...
    "optconc.desc": "Run this recognizer across N worker processes (each B=1, fills idle GPU). VRAM scales with N. The same number of images run detect+recognize at once, filling the pool with each other's crops. Blank = server default, 1 = no pool (serial).",
    "btn.save": "Save",
    ...
    "plugins.restartNeeded": "Restart the server for the new code to take effect (the running process still holds the old module).",
    ...
    "stats.filter": "Engine combo",
    "stats.filter.all": "All (combos mixed)",
    "stats.filter.desc": "Recognizer speed differs sharply per combo, so a pooled average represents none of them. Read one at a time.",
    ...
    "behavior.minFont.label": "Min font size (px)",
    "behavior.minFont.desc": "Overlay text is sized to the largest size that fits its box, but never below this. Text that still overflows is clipped; hover to see the full sentence. The extension picks this up on connect.",
    ...
    "behavior.idleUnload.label": "Idle detect/recognize model unload (min)",
    ...
    "field.unknownChoice": "(unsupported)",
    ...
    "toast.reinstalled": "{name} reinstalled — restart required",
