...
# Source languages whose comics lay their panels/bubbles out right-to-left. Japanese
# manga does; Korean webtoons, English comics and mainland Chinese webcomics read
# left-to-right. Absent = left-to-right, so an unknown language degrades that way.
# This orders the regions the pipeline hands the translator, which reads a page's
# bubbles as one sequence — so it is the source page's layout that decides, not the
# destination language.
LANG_RTL: frozenset[str] = frozenset({"ja"})
