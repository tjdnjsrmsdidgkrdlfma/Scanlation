...
- **Docker(P7) — 라이브 배포 완료.** core-only 이미지가 리눅스 호스트에서 `docker compose up`으로 기동,
  `/admin` 원클릭으로 `ctd`+`mangaocr`+`ollama`를 **공개 레포에서 자격증명 없이** 설치(볼륨 영속 → 재시작
...
> `git subtree split --prefix=packages/scanlation-ctd -b ctd-only`. 근거: [SCANLATION_DESIGN.md](SCANLATION_DESIGN.md).
...
1. **패키지(코드)** — 엔진 플러그인은 별도 pip 패키지다. bare-metal은 `pip install`로, **Docker/런타임은
   `/admin` 원클릭**으로 설치한다: `POST /install_plugins/ {"plugins": {"ctd": true}}`가 미설치 시 엔진을
...
- **검출 노이즈/과분할 (P8 1차 완료)**: `min_side`(짧은변 하한) + 원본 px `min_area`로 SFX 조각·
  말줄임표(`．．．`)·작은 하트를 컷하고, `merge_aspect`(세로 편향 커널)로 세로 컬럼이 조각나지 않게
  병합(실측 예: 37→18박스). 큰 장식 하트는 아직 일부 남고, 문장부호-only OCR 스킵은 미구현.
- **검출 옵션 추가 후보(미구현, P8 예정)** — 현재는 하한(`min_area`/`min_side`)만 있음. 넣을 만한 것:
  상한 `max_area`/`max_area_ratio`(말풍선·패널 통짜 오검출 컷), `fill_ratio`(윤곽 픽셀÷박스 면적
  하한 — morph-close가 성기게 이어붙인 빈 박스 걸러냄), 극단 종횡비 컷, 큰 장식 하트 억제.
