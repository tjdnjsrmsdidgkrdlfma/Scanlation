...
가능한 플러그인입니다. (현 기본 검출기 `rtdetr`는 **축정렬** 박스만 내므로 지금은 deskew가
...
만화 페이지에서 `rtdetr`(**GPU**) 검출 → `mangaocr`(CPU) 일본어 OCR → `ollama`(gemma4-26b, **GPU**)
**실제 한국어 번역**까지. 브라우저 오버레이도 별도 검증(로컬 http). Docker 배포(P7)도
...
| P3 | `rtdetr` 검출기(RT-DETR text/bubble 클래스 검출 + text-class 필터 + IoU/IoS 중복제거, 축정렬 박스) + `visualize.py` | ✅ 실페이지 검증 |
...
**검증됨:**
- **리눅스/gfx1200 호스트** — 실제 Pixiv 페이지 → `rtdetr,mangaocr,ollama` → 진짜 ja→ko. 예:
  `背中のチャックを閉めてもらってもいいですか…？` → `등에 있는 지퍼 좀 올려주실 수 있을까요...?`
- **브라우저(윈도우)** — `rtdetr` 검출 + `mangaocr` OCR + `%` 오버레이, 실제 862×1200 페이지.
- 서버 설정(엔진·모델·언어·프롬프트)은 전부 **`/admin`**에서(→ `state.json` 영속). 실행 명령엔 env·플래그 없음.
- 코어 단위테스트 **34개** 통과(`python -m tests`, pytest 미사용·모델/GPU 불필요); 엔진 패키지는
  각자 suite(`rtdetr`/`mangaocr` 스모크는 가중치/패키지 없으면 자동 skip, `ollama`/`llamacpp`는 HTTP mock).
- **Docker(P7) — 라이브 배포 완료.** core-only 이미지가 리눅스 호스트에서 `docker compose up`으로 기동,
  `/admin` 원클릭으로 `rtdetr`+`mangaocr`+`ollama`를 **공개 레포에서 자격증명 없이** 설치(볼륨 영속 → 재시작
...
```
[ MV2 확장 ]  ──HTTP (md5 / box / lazy)──►  [ FastAPI 서버 ]
 이미지 발견                                 detect ─► deskew ─► recognize ─► translate
 + %-오버레이                                (rtdetr)  (geometry) (mangaocr)  (ollama|llamacpp)
```

- **서버** ([packages/scanlation-server/](packages/scanlation-server/)): FastAPI + uvicorn(단일 워커),
  async GPU 락 1개, 블로킹 모델 작업은 threadpool. SQLite 결과 캐시 + 번역 기록(TM). 엔진은
...
- `POST /run_pipeline/`는 **lazy**(`{md5, options}` → 캐시 히트, 미스 시 non-2xx) 후
  **work**(`{md5, contents, options}`). 그 외: `/set_engines/`, `/set_languages/`,
  `/install_plugins/`, `/clear_cache/`, 그리고 `GET /` handshake.
...
packages/
  ...
  scanlation-rtdetr/    detector 플러그인 (transformers + torch)
...
> `git subtree split --prefix=packages/scanlation-rtdetr -b rtdetr-only`. 근거: [SCANLATION_DESIGN.md](SCANLATION_DESIGN.md).
...
./venv/Scripts/python -m pip install -e ./packages/scanlation-sdk -e ./packages/scanlation-server \
  -e ./packages/scanlation-rtdetr -e ./packages/scanlation-mangaocr \
...
```
또는 `models/rtdetr/`에 transformers 스냅샷(config.json + preprocessor_config.json + model.safetensors)을
직접 배치 / `SCANLATION_RTDETR_MODEL=/path/to/dir` 로 디렉터리 지정. (ollama/llamacpp는 별도 서비스라
...
| recognizer | `mangaocr` | (없음) |
| translator | `ollama`, `llamacpp` | (없음) |
...
1. **패키지(코드)** — 엔진 플러그인은 별도 pip 패키지다. bare-metal은 `pip install`로, **Docker/런타임은
   `/admin` 원클릭**으로 설치한다: `POST /install_plugins/ {"plugins": {"rtdetr": true}}`가 미설치 시 엔진을
   **GitHub에서 `pip install`**(`git+<repo>@<ref>#subdirectory=packages/scanlation-rtdetr`)해
...
2. **가중치** — 패키지가 깔린 뒤 `rtdetr`(transformers 스냅샷)·`mangaocr`(HF) 가중치를 이어서 다운로드(같은 원클릭이 연달아 수행,
...
| `SCANLATION_DETECTOR` / `_RECOGNIZER` / `_TRANSLATOR` | `rtdetr` / (빈 값) / (빈 값) | 최초 기동 기본 엔진(detector 기본 `rtdetr`; 나머지 빈 값=미선택; 이후 `/admin`이 덮어씀) |
...
| `SCANLATION_RTDETR_MODEL` | — | RT-DETR transformers 스냅샷 **디렉터리** 명시 경로(미설정 시 `models/rtdetr/`, `install()`이 HF 레포에서 다운로드) |
...
> **ollama 동시성**: 번역은 이미지 단위 배치로 GPU 락 밖에서 여러 이미지를 **동시에** 처리합니다(`SCANLATION_TRANSLATE_CONCURRENCY`, 기본 4). 이를 GPU에서 실제 병렬로 생성하려면 **호스트 ollama 데몬**의 `OLLAMA_NUM_PARALLEL`을 맞춰 켜야 합니다 — 설정 방법·VRAM 계산은 아래 **배포 → 호스트 ollama 튜닝** 참고.
...
```bash
cd packages/scanlation-server
../../venv/Scripts/python tools/visualize.py page.jpg --detector rtdetr --out annotated.png   # 박스 + 인덱스
```
...
- **gfx1200(RDNA4) ROCm**이 최대 리스크 — 사전빌드 휠 부재 가능. 완화: `rtdetr`는 CUDA/ROCm torch가
...
```
팝업 Server = `http://127.0.0.1:4001` → Connect (드롭다운에 `rtdetr/mangaocr/ollama`).
...
```
1. **엔진 설치** — `http://127.0.0.1:4010/admin` 플러그인 탭에서 `rtdetr`·`mangaocr`(+원하면 `ollama`/`llamacpp`)
...
던진다(`SCANLATION_TRANSLATE_CONCURRENCY`, 기본 4). 이 동시 요청을 GPU에서 **실제 병렬로 생성**하려면
아래를 **호스트 ollama 데몬**에 설정해야 한다(우리 compose가 아니라 ollama 자체 env — 컨테이너 밖에 있음):

| ollama env | 권장 | 의미 |
|---|---|---|
| `OLLAMA_NUM_PARALLEL` | `4` | 한 모델이 동시에 처리할 요청 수. 서버 `SCANLATION_TRANSLATE_CONCURRENCY`와 맞춘다 |
...
4. 번역은 **이미지 단위 전체 배치 + GPU 락 밖 실행**(말풍선 상호 문맥 → 일관성, 이미지 간 병렬).
   ...
   (위 배포 → **호스트 ollama 튜닝** 참고). 서버 쪽 상한은 `SCANLATION_TRANSLATE_CONCURRENCY`(기본 4).
