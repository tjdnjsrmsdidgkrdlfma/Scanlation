...
- 단위테스트 **37개** 통과(`python -m tests`, pytest 미사용·모델/GPU 불필요); `ctd`/`mangaocr`
  스모크는 개별 실행(`python -m tests.test_ctd`)이며 가중치/패키지 없으면 자동 skip.
...
server/
  ...
  tests/      단위 37개(핸드롤 러너, `python -m tests`) + 개별 모델 스모크
...
```bash
cd server
../venv/Scripts/python -m tests                # 빠른 단위 37개, 모델/GPU 불필요 (pytest 미사용, 자체 러너)
