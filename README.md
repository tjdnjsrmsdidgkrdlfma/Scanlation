...
- 코어 단위테스트 **31개** 통과(`python -m tests`, pytest 미사용·모델/GPU 불필요); 엔진 패키지는
...
- **유지보수** — **페이지 캐시 비우기**(`POST /clear_cache/`): 저장된 페이지 결과(`ocr_runs`)를 모두 지워
  다음 접속 때 전 과정을 재실행. 수동 교정(번역 메모리)은 보존.
