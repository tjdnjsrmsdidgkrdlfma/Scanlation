...
- **Docker(P7) — 라이브 배포 완료.** core-only 이미지가 리눅스 호스트에서 `docker compose up`으로 기동,
  ...
  `127.0.0.1:4010`만 publish). 라이브 handshake + `/run_tsl/` ja→ko 검증됨.
...
- `POST /run_ocrtsl/`는 **lazy**(`{md5, options}` → 캐시 히트, 미스 시 non-2xx) 후
  **work**(`{md5, contents, options}`). 그 외: `/run_tsl/`, `/get_trans/`,
  `/set_models/`, `/set_lang/`, `/get_active_options/`,
  `/get_plugin_data/`, `/manage_plugins/`, `/clear_cache/`, 그리고 `GET /` handshake.
