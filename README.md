...
> **ollama 동시성**: 번역은 이미지 단위 배치로 GPU 락 밖에서 여러 이미지를 **동시에** 처리합니다(동시 이미지 수는 `/admin` **동작** 탭의 "동시 번역 이미지 수", 기본 4 — `state.json`에 영속, 재시작 없이 조절). 이를 GPU에서 실제 병렬로 생성하려면 **호스트 ollama 데몬**의 `OLLAMA_NUM_PARALLEL`을 맞춰 켜야 합니다 — 설정 방법·VRAM 계산은 아래 **배포 → 호스트 ollama 튜닝** 참고.
...
던진다(동시 이미지 수는 `/admin` **동작** 탭의 "동시 번역 이미지 수", 기본 4). 이 동시 요청을 GPU에서 **실제 병렬로 생성**하려면
...
4. 번역은 **이미지 단위 전체 배치 + GPU 락 밖 실행**(말풍선 상호 문맥 → 일관성, 이미지 간 병렬).
   ...
   (위 배포 → **호스트 ollama 튜닝** 참고). 서버 쪽 상한은 `/admin` 동작 탭의 "동시 번역 이미지 수"(기본 4).
