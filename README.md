...
**검증됨:**
- **리눅스/gfx1200 CLI** — 실제 Pixiv 페이지 → `ctd,mangaocr,ollama` → 진짜 ja→ko. 예:
  `背中のチャックを閉めてもらってもいいですか…？` → `등에 있는 지퍼 좀 올려주실 수 있을까요...?`
- **브라우저(윈도우)** — `ctd` 검출 + `mangaocr` OCR + `%` 오버레이, 실제 862×1200 페이지.
- 단위테스트 **35개** 통과(`python -m tests`, 모델/GPU 불필요); `ctd`/`mangaocr` 스모크는
  개별 실행(`python -m tests.test_ctd`)이며 가중치/패키지 없으면 자동 skip.
...
```bash
python -m venv venv
# Windows: ./venv/Scripts/python -m pip install -e "./server[ctd,mangaocr]"
# Linux:   source venv/bin/activate && pip install -e "./server[ctd,mangaocr]"
```
...
```bash
cd server
../venv/Scripts/python -m tests                # 빠른 단위 35개, 모델/GPU 불필요 (pytest 미사용, 자체 러너)
../venv/Scripts/python -m tests.test_ctd       # CTD 스모크 (가중치 없으면 자동 skip)
../venv/Scripts/python -m tests.test_mangaocr  # manga-ocr 스모크 (패키지 없으면 자동 skip)
```
...
2. `pip install -e "./server[ctd,mangaocr]"` 후 `cd server && python -m tests` green 확인.
